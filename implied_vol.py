...
def fetch_chains(ticker=TICKER, n_expiries=N_EXPIRIES):
    ...
    Expiries are chosen spread out in time (at least ~5 trading days apart)
    rather than simply the nearest n_expiries in the list. Exchanges have
    added increasingly frequent weekly and even daily expirations, and the
    newest of these often carry near-zero open interest because they were
    just listed. Taking the chronologically nearest n_expiries can therefore
    return a cluster of illiquid dates within the same week rather than a
    spread across the month, which is what this analysis needs to study how
    the skew evolves with time to expiration.

    Note on timing: this pulls a live snapshot, so results depend on when it
    is run. Quotes are thinnest, and open interest filters bite hardest,
    outside of U.S. market hours (9:30 AM-4:00 PM Eastern, Monday-Friday).
    Running this during market hours will produce more liquid, representative
    results than running it overnight or on a weekend.
    """
    tk = yf.Ticker(ticker)
    spot = float(tk.fast_info["lastPrice"])
    today = pd.Timestamp.now().normalize()

    now_et = pd.Timestamp.now(tz="UTC").tz_convert("America/New_York")
    is_market_hours = (
        now_et.weekday() < 5
    ...
    if not is_market_hours:
        print(f"  Note: current time is {now_et:%Y-%m-%d %H:%M} ET, outside "
              "regular market hours. Option quotes may be thin or stale; "
              "for the most representative results, re-run during market hours.")
    ...
    last_days = -999
    for e in candidates:
        days = (pd.Timestamp(e) - today).days
        if days - last_days >= 5:
            expiries.append(e)
            last_days = days
...
def plot_smile(df, spot, ticker, path):
    ...
    if len(sub) < 5:
        raise ValueError(
            f"Only {len(sub)} liquid OTM contracts found for {expiry}. "
            "Today's market has unusually thin liquidity at this expiry; "
            "try a different expiry or re-run later in the trading day."
        )
